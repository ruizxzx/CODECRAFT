...
const questionPublic=(data:any,idv:string):SocialQuestion=>({id:idv,title:String(data.title||''),details:String(data.details||''),authorId:data.isAnonymous?undefined:data.authorId,authorUsername:data.isAnonymous?undefined:data.authorUsername,authorName:data.isAnonymous?'Anonymous':(data.authorName||'Anonymous'),authorAvatar:data.isAnonymous?'':(data.authorAvatar||''),isAnonymous:!!data.isAnonymous,topics:Array.isArray(data.topics)?data.topics:[],attachments:Array.isArray(data.attachments)?data.attachments:[],relatedArticleSlug:data.relatedArticleSlug||undefined,relatedCommunityId:data.relatedCommunityId||undefined,followersCount:Number(data.followersCount||0),answersCount:Number(data.answersCount||0),upvotesCount:Number(data.upvotesCount||0),bestAnswerId:data.bestAnswerId||undefined,moderationState:(data.moderationState||'VISIBLE') as QuestionModerationState,duplicateOf:data.duplicateOf||undefined,isVerified:!!data.isVerified,verificationColor:String(data.verificationColor||'#2196F3'),createdAt:date(data.createdAt),updatedAt:date(data.updatedAt)});
