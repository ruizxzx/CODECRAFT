...
export const SearchModal: React.FC<SearchModalProps> = ({ isOpen, onClose, articles, onSelectArticle, onNavigate, siteConfig }) => {
  ...
  const hasUnsupportedCommentFilter = Boolean(plan.filters.topic || plan.filters.tag || plan.filters.community || plan.filters.series || plan.filters.before || plan.filters.after);
  const matchingComments = useMemo(() => {
    if (!facetTypeAllowed(['comment']) || hasUnsupportedCommentFilter) return [];
  ...
  }, [comments, normalized, activeType, plan.filters.author, hasUnsupportedCommentFilter]);
