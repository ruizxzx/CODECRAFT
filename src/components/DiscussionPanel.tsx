...
export const DiscussionPanel: React.FC<Props> = ({ post, user, onNavigate, onQuote }) => {
  ...
  const makeSummary = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/ai/discussion-summary',{method:'POST',headers:{'Content-Type':'application/json',...(token?{Authorization:`Bearer ${token}`}:{})},body:JSON.stringify({title:post.title,content:post.content,replies:comments.slice(0,120).map(c=>({id:c.id,author:c.authorUsername,content:c.content,likes:c.likeCount||0,authorResponse:!!(c.isAuthorResponse||c.authorId===post.authorId)}))})});
      const data=await response.json(); if(!response.ok) throw new Error(data?.error||'AI summary failed.'); setSummary(data); notifyToast('Discussion intelligence generated.','success');
