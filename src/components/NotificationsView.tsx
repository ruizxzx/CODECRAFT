...
export const NotificationsView: React.FC<Props> = ({ userProfile, onNavigate }) => {
  ...
  useEffect(() => {
    if (!userProfile) { setItems([]); setLoading(false); setActorAvatars({}); return; }
    setLoading(true);
  ...
  useEffect(() => {
    ...
    const resolve = async () => {
      ...
      if (!ids.length) { setActorAvatars({}); return; }
      const next: Record<string,string> = {};
      await Promise.all(ids.map(async uid => {
        try { const p = await getCommunityProfile(uid); if (p?.photoURL) next[uid] = p.photoURL; } catch { /* stale notification data is a safe fallback */ }
      }));
      if (active) setActorAvatars(next);
    };
    void resolve();
    return () => { active = false; };
  }, [items]);
  ...
  return <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10 sm:py-16">
    ...
    {loading ? <div className="py-20 text-center font-mono text-xs uppercase">SYNCING NOTIFICATIONS…</div> : items.length === 0 ? <div className="border-4 border-black p-10 text-center font-mono text-sm">NO NOTIFICATIONS YET.</div> : <div className="space-y-6">{groupedRows.map((row:any)=> row.kind==='day' ? <div key={row.key} className="font-mono text-[10px] font-black uppercase border-b-2 border-black pb-2">{row.key}</div> : row.kind==='batch' ? <button key={row.key} onClick={()=>row.list?.forEach((n:Notification)=>open(n))} className="w-full text-left border-2 border-black bg-white p-4 hover:bg-neutral-50"><div className="font-mono text-xs font-black">{row.list?.length} people {row.list?.[0]?.message || 'interacted with your content'}</div><div className="font-mono text-[10px] text-neutral-500 mt-1">{row.list?.slice(0,3).map((n:Notification)=>`@${n.actorUsername}`).join(', ')}{row.list?.length>3?' + more':''}</div></button> : <button key={row.key} onClick={()=>open(row.item!)} className={`w-full text-left border-2 border-black p-4 flex gap-3 hover:bg-neutral-50 ${row.item?.read?'bg-white':'bg-[var(--color-secondary)]/20'}`}><div className="w-10 h-10 border-2 border-black rounded-full overflow-hidden shrink-0">{(row.item?.actorId && actorAvatars[row.item.actorId]) || row.item?.actorAvatar ? <img src={(row.item?.actorId && actorAvatars[row.item.actorId]) || row.item?.actorAvatar || ''} className="w-full h-full object-cover"/>:<Bell className="w-full h-full p-2"/>}</div><div className="min-w-0 flex-1"><div className="font-mono text-xs"><b>@{row.item?.actorUsername}</b> {row.item?.message}</div><div className="text-[10px] text-neutral-500 mt-1">{formatTime(row.item?.createdAt)}</div></div>{!row.item?.read&&<Check className="ml-auto w-4 h-4 shrink-0"/>}</button>)}</div>}
  </div>;
};
