...
export const ExploreView: React.FC<Props> = ({ articles, userAuth, userProfile, onNavigate, initialHashtag = '' }) => {
  ...
  const items = useMemo<ExploreItem[]>(() => {
    ...
    return result;
  }, [articles, posts, series, query, followingIds, followingUsernames, hidden]);
        ...
        <section className="mb-7 overflow-x-auto">
          <div className="flex gap-2 min-w-max">
            ...
            {trendingTopics.map(([tag,count]) => (
              <button key={tag} onClick={()=>setQuery(`#${tag}`)} className="px-3 py-2 border-2 border-black bg-white font-mono text-[10px] font-black hover:bg-[var(--color-secondary)]">#{tag} · {count}</button>
            ))}
          </div>
        </section>
      )}
            ...
            {visibleItems.map(item => {
                ...
                return (
                  <article key={item.key} className="bg-white border-4 border-black neo-shadow-sm overflow-hidden">
                    ...
                    <div className="p-5 sm:p-6">
                      ...
                      <div className="flex flex-wrap gap-2 mt-4">{a.tags.slice(0,5).map(tag=><button key={tag} onClick={()=>setQuery(`#${tag}`)} className="font-mono text-[10px] border border-black px-2 py-1 hover:bg-[var(--color-secondary)]">#{tag}</button>)}</div>
                      <div className="flex items-center justify-between gap-4 mt-5 pt-4 border-t-2 border-black">
                        ...
                        <div className="flex gap-2">
                          <button onClick={()=>explainItem(item)} className="px-2 py-2 border-2 border-black font-mono text-[9px] uppercase hover:bg-neutral-100">Why this?</button>
                          <button onClick={()=>hideItem(item.key)} className="p-2 border-2 border-black hover:bg-red-100" title="Not interested"><X className="w-4 h-4"/></button>
                  ...
                  <article key={item.key} className="bg-[var(--color-primary)] border-4 border-black neo-shadow-sm overflow-hidden">
                    <div className="grid md:grid-cols-[220px_1fr]">
                      ...
                      <div className="p-5 sm:p-6">
                        ...
                        <div className="flex items-center justify-between gap-4 mt-5 pt-4 border-t-2 border-black"><button onClick={()=>onNavigate('community_profile',s.ownerUsername || '')} className="font-mono text-xs font-black uppercase inline-flex items-center gap-2"><UserPlus className="w-4 h-4"/> @{s.ownerUsername || 'creator'}</button><button onClick={()=>onNavigate('series',s.id)} className="px-4 py-2 bg-black text-white border-2 border-black font-display font-black uppercase">OPEN SERIES →</button></div>
                      </div>
                    </div>
                  </article>
                );
              ...
              return (
                <article key={item.key} className="bg-white border-4 border-black neo-shadow-sm p-5 sm:p-6">
                  <div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2 font-mono text-[10px] uppercase"><span className="px-2 py-1 bg-[var(--color-secondary)] border-2 border-black font-black">{p.type==='blog'?'COMMUNITY BLOG':'POST'}</span><span>{formatDisplayDate(p.createdAt)}</span></div><button onClick={()=>hideItem(item.key)} className="p-1 border-2 border-transparent hover:border-black"><MoreHorizontal className="w-4 h-4"/></button></div>
                  <button onClick={()=>onNavigate('community_post',p.id)} className="text-left w-full mt-3"><h2 className="font-display font-black text-2xl sm:text-3xl uppercase leading-tight hover:text-[var(--color-secondary)]">{p.title}</h2><p className="text-sm text-neutral-700 mt-2 line-clamp-4 whitespace-pre-wrap">{p.excerpt || p.content}</p></button>
                  ...
                  <div className="flex items-center justify-between gap-4 mt-5 pt-4 border-t-2 border-neutral-200">
                    <button onClick={()=>onNavigate('community_profile',p.authorUsername)} className="flex items-center gap-2 text-left"><div className="w-8 h-8 rounded-full border-2 border-black overflow-hidden bg-neutral-100">{p.authorAvatar&&<img src={p.authorAvatar} alt="" className="w-full h-full object-cover"/>}</div><span><span className="block font-display font-black text-xs uppercase">{p.authorName || p.authorUsername} <VerifiedBadge verified={p.isVerified} color={p.verificationColor} className="inline w-3.5 h-3.5"/></span><span className="block font-mono text-[9px] text-neutral-500">@{p.authorUsername}</span></span></button>
