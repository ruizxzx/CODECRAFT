...
export interface CommunityFeedPost { id:string; communityId?:string; parentPostId?:string; postType?:'blog'|'discussion'|'question'|'link'|'poll'|'announcement'; title:string; content:string; authorId:string; authorUsername:string; authorName:string; authorAvatar:string; coverImage?:string; coverImageAlt?:string; excerpt?:string; authorPlatformRole?:'member'|'moderator'|'master_admin'; score:number; commentsCount:number; isFeatured?:boolean; isPinned?:boolean; isLocked?:boolean; isArchived?:boolean; flair?:string; dedupeKey?:string; linkUrl?:string; mediaUrls?:string[]; poll?:{question:string;options:string[];votes?:Record<string,number>}; createdAt:string; updatedAt?:string; editedAt?:string; seriesId?:string; seriesName?:string; seriesOrder?:number; }
export interface SocialQuestion { id:string; title:string; details:string; authorId:string; authorUsername:string; authorName:string; authorAvatar:string; topics:string[]; followersCount:number; answersCount:number; upvotesCount:number; bestAnswerId?:string; mediaUrls?:string[]; coverImage?:string; coverImageAlt?:string; attachments?:Array<{url?:string;downloadUrl?:string;src?:string;type?:string;name?:string}>; moderationState?:string; isAnonymous?:boolean; createdAt:string; updatedAt?:string; }
...
export async function createQuestion(user:CommunityUser,title:string,details:string,topics:string[],options:{mediaUrls?:string[];coverImage?:string;coverImageAlt?:string;attachments?:Array<{url?:string;downloadUrl?:string;src?:string;type?:string;name?:string}>}={}){ const qid=id(); const mediaUrls=Array.from(new Set((options.mediaUrls||[]).filter(Boolean).map(String))).slice(0,4); const attachments=(options.attachments||[]).filter((a:any)=>a&&typeof a==='object').slice(0,4); const data:any={title:title.trim().slice(0,256),details:details.trim().slice(0,100000),authorId:user.uid,authorUsername:user.username,authorName:user.displayName,authorAvatar:user.photoURL||'',topics:topics.map(x=>x.toLowerCase().trim()).filter(Boolean).slice(0,10),followersCount:0,answersCount:0,upvotesCount:0,moderationState:'VISIBLE',isAnonymous:false,createdAt:serverTimestamp(),updatedAt:serverTimestamp()}; if(mediaUrls.length)data.mediaUrls=mediaUrls; if(options.coverImage)data.coverImage=String(options.coverImage).slice(0,2000); if(options.coverImageAlt)data.coverImageAlt=String(options.coverImageAlt).slice(0,300); if(attachments.length)data.attachments=attachments; await setDoc(doc(db,'questions',qid),data); return {...data,id:qid,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()} as SocialQuestion; }
export async function updateQuestion(qid:string,uid:string,title:string,details:string){ const q=await getDoc(doc(db,'questions',qid));if(!q.exists())throw new Error('Question not found.');const admin=await resolveMasterAccess();if(q.data().authorId!==uid&&!admin)throw new Error('You cannot edit this question.');await updateDoc(q.ref,{title:title.trim().slice(0,256),details:details.trim().slice(0,100000),updatedAt:serverTimestamp()}); }
export async function deleteQuestion(qid:string,uid:string){ const q=await getDoc(doc(db,'questions',qid));if(!q.exists())return;const admin=await resolveMasterAccess();if(q.data().authorId!==uid&&!admin)throw new Error('You cannot delete this question.');const a=await getDocs(collection(db,'questions',qid,'answers'));const v=await getDocs(collection(db,'questions',qid,'votes'));const f=await getDocs(collection(db,'questions',qid,'followers'));const b=writeBatch(db);a.docs.forEach(d=>b.delete(d.ref));v.docs.forEach(d=>b.delete(d.ref));f.docs.forEach(d=>b.delete(d.ref));b.delete(q.ref);await b.commit(); }
