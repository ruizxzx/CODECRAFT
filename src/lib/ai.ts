...
const localCacheKey = (task:AITask,input:AIContentInput,options:Record<string,unknown>) => {
  const raw = JSON.stringify({task,id:input.contentId||'',title:input.title,content:input.content.slice(0,12000),revision:input.sourceRevision||'',options});
  let h=2166136261; for(let i=0;i<raw.length;i++){h^=raw.charCodeAt(i);h=Math.imul(h,16777619);} return `offscrpt:ai:v2:${task}:${(h>>>0).toString(36)}`;
};
...
export function clearAICache(){ try { const keys=[]; for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i);if(k?.startsWith('offscrpt:ai:v2:'))keys.push(k);} keys.forEach(k=>localStorage.removeItem(k)); } catch (error) { console.warn('AI cache clear skipped:', error); } }
