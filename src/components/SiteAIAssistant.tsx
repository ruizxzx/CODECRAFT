...
type Props={open:boolean;onClose:()=>void;articles:Article[];currentPage?:PageView;currentContent?:AIContentInput;onNavigate:(page:PageView,param?:string)=>void;embedded?:boolean};
export const SiteAIAssistant:React.FC<Props>=({open,onClose,articles,currentPage,currentContent,onNavigate,embedded=false})=>{
 ...
 if(!open)return null;
 const getRetrievalScope=()=>mode;
 const getSavedRetrievalIds=()=>{try{const uid=auth.currentUser?.uid;if(!uid)return [];const a=uid?JSON.parse(localStorage.getItem(`offscrpt:saved:articles:${uid}:v2`)||'[]'):[];const p=uid?JSON.parse(localStorage.getItem(`offscrpt:saved:posts:${uid}:v2`)||'[]'):[];return [...(Array.isArray(a)?a:[]).map((id:string)=>({type:'article',id:String(id)})),...(Array.isArray(p)?p:[]).map((id:string)=>({type:'post',id:String(id)}))];}catch{return [];}};
 const retrievalCurrent=currentContent?{type: currentContent.contentType==='community-blog'||currentContent.contentType==='community_post'?'post':currentContent.contentType, id: currentContent.contentId}:undefined;
 const retrievalOptions=(question:string)=>({question, mode:'standard', conversation:messages.slice(-12), offscrptRetrieval:{enabled:true,scope:getRetrievalScope(),current:retrievalCurrent,savedIds:getSavedRetrievalIds(),limit:10}});
 const ask=async()=>{const question=q.trim();if(!question){notifyToast('Enter a question first.','error');return;}setBusy(true);try{
   ...
   const result:any=await requestAI('ask',base,retrievalOptions(question));
 ...
 return <div className={`ai-themed ${embedded ? "w-full" : "fixed inset-0 z-[110] bg-black/70 flex items-start justify-center p-4 sm:p-8 overflow-auto"}`} role="dialog" aria-modal={!embedded} aria-label="OFFSCRPT AI">
  <div className={`w-full max-w-4xl bg-white border-4 border-black neo-shadow-lg ${embedded ? "" : "mt-8"}`}>
   <div className="bg-[var(--color-primary)] border-b-4 border-black p-5 flex items-center justify-between gap-3"><div className="flex items-center gap-3"><Bot className="w-6 h-6"/><div><div className="font-mono text-[9px] font-black">OFFSCRPT INTELLIGENCE</div><h2 className="font-display font-black text-3xl uppercase">OFFSCRPT AI</h2></div></div>{!embedded&&<button onClick={onClose} className="border-2 border-black bg-white px-3 py-2 font-mono text-[9px] font-black">CLOSE</button>}</div>
   <div className="p-5 space-y-4">
    ...
    <div className="font-mono text-[8px] text-neutral-500 uppercase">Context: {mode==='site'?'OFFSCRPT retrieval':mode==='page'?'current page + OFFSCRPT retrieval':'your saved content + permission-checked retrieval'}. Source access is server-authorized.</div>
   </div>
  </div>
 </div>;
};
