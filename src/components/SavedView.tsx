    ...
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-14">
      ...
      <div className="bg-white border-4 border-black p-6 sm:p-8 neo-shadow mb-8">
        ...
        <div className="mt-5 pt-5 border-t-2 border-black">
          ...
          {showCollectionForm && userAuth && <form onSubmit={async e => { e.preventDefault(); if (!newCollectionName.trim()) return; try { const c=await createBookmarkCollection(userAuth.uid,newCollectionName); setCollections(prev=>[...prev,c]); setNewCollectionName(''); setShowCollectionForm(false); setActiveCollection(c.id); } catch(err:any){ notifyToast(err?.message || 'Could not create collection'); } }} className="flex gap-2 mt-3 max-w-lg"><input value={newCollectionName} onChange={e=>setNewCollectionName(e.target.value)} maxLength={50} placeholder="e.g. AI research" className="flex-1 border-2 border-black px-3 py-2 font-mono text-xs"/><button className="px-3 py-2 bg-black text-white border-2 border-black font-mono text-xs font-bold">CREATE</button></form>}
          {userAuth && activeCollection !== 'all' && activeCollection !== 'general' && <button onClick={async()=>{if(!confirm('Delete this collection? Saved items will move to General.'))return; try{await deleteBookmarkCollection(userAuth.uid,activeCollection); setCollections(prev=>prev.filter(c=>c.id!==activeCollection)); setActiveCollection('all');}catch(e){notifyToast('Could not delete collection.')}}} className="mt-2 font-mono text-[10px] font-bold uppercase underline">Delete collection</button>}
        </div>
      </div>
