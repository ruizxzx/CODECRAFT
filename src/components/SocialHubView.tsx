...
type PostType = 'blog' | 'discussion' | 'question' | 'link' | 'poll' | 'announcement';
...
export const SocialHubView: React.FC<Props> = ({ userProfile, onNavigate }) => {
  ...
  const createP = async (parentPostId?: string) => {
    if (!requireAuth() || !selected || !title.trim() || !content.trim() || posting) return;
...
const CommunityPanel: React.FC<CommunityPanelProps> = props => {
  ...
  const [member, setMember] = useState(false); const [role, setRole] = useState(''); const [postKind, setPostKind] = useState<'all'|'blog'|'discussion'|'question'|'link'|'poll'|'announcement'>('all');
  ...
  const roots = posts.filter(p => !p.parentPostId && (postKind === 'all' || (p.postType || 'discussion') === postKind));
  const children = (id: string) => posts.filter(p => p.parentPostId === id);
  const visibleCount = posts.filter(p => !p.parentPostId && (postKind === 'all' || (p.postType || 'discussion') === postKind)).length;
