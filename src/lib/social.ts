...
export type CommunityMembershipMode='public'|'private'|'invite-only'|'approval-required';
export interface CommunityJoinRequest { id:string; communityId:string; userId:string; username?:string; status:'pending'|'approved'|'rejected'|'cancelled'; createdAt:string; updatedAt?:string; }
export interface CommunityInvitation { id:string; communityId:string; inviteeId:string; inviteeUsername?:string; inviterId:string; inviterUsername?:string; status:'pending'|'accepted'|'declined'|'revoked'; createdAt:string; }
export interface CommunityWikiPage { id:string; title:string; slug:string; content:string; authorId:string; updatedAt:string; createdAt:string; archived?:boolean; }
export interface CommunityFaq { id:string; question:string; answer:string; order:number; updatedAt:string; updatedBy:string; }
export interface CommunityResource { id:string; title:string; description?:string; url:string; category?:string; creatorId:string; createdAt:string; }
export interface SocialCommunity { id:string; name:string; slug:string; description:string; iconUrl?:string; bannerUrl?:string; rules?:string[]; membersCount:number; postsCount:number; ownerId:string; ownerUsername?:string; ownerName?:string; ownerAvatar?:string; createdAt:string; updatedAt?:string; isPrivate?:boolean; membershipMode?:CommunityMembershipMode; wikiEnabled?:boolean; isArchived?:boolean; isLocked?:boolean; allowLinks?:boolean; allowMedia?:boolean; defaultPostType?:'discussion'|'question'|'link'|'poll'; }
...
export async function getCommunityMembershipMode(cid:string):Promise<CommunityMembershipMode>{const c=await getCommunity(cid);return c?.membershipMode || (c?.isPrivate?'private':'public');}
export async function setCommunityMembershipMode(cid:string,mode:CommunityMembershipMode,actorId:string){const c=await getCommunity(cid);if(!c||c.ownerId!==actorId)throw new Error('Only the community owner can change membership mode.');await updateDoc(doc(db,'communities',cid),{membershipMode:mode,isPrivate:mode==='private',updatedAt:serverTimestamp()});}
export async function joinCommunity(cid:string,user:CommunityUser){if(!user?.uid)throw new Error('Sign in required.');const community=await getCommunity(cid);if(!community)throw new Error('Community not found.');const profile=await getDoc(doc(db,'users',user.uid));if(profile.data()?.isBlocked===true)throw new Error('Your account cannot join communities.');const mode=community.membershipMode||(community.isPrivate?'private':'public');if(mode==='approval-required'){await requestCommunityJoin(cid,user);return 'pending';}if(mode==='invite-only')throw new Error('This community requires an invitation.');const ref=doc(db,'communities',cid,'members',user.uid);if((await getDoc(ref)).exists())return false;const b=writeBatch(db);b.set(ref,{uid:user.uid,username:user.username,role:'member',createdAt:serverTimestamp(),updatedAt:serverTimestamp()});b.update(doc(db,'communities',cid),{membersCount:increment(1),updatedAt:serverTimestamp()});await b.commit();return true;}
export async function requestCommunityJoin(cid:string,user:CommunityUser){if(!user?.uid)throw new Error('Sign in required.');const community=await getCommunity(cid);if(!community)throw new Error('Community not found.');const mode=community.membershipMode||(community.isPrivate?'private':'public');if(mode!=='approval-required')throw new Error('This community does not require approval.');const memberRef=doc(db,'communities',cid,'members',user.uid);if((await getDoc(memberRef)).exists())return;const profile=await getDoc(doc(db,'users',user.uid));if(profile.data()?.isBlocked===true)throw new Error('Your account cannot join communities.');const ref=doc(db,'communities',cid,'joinRequests',user.uid);const s=await getDoc(ref);if(s.exists()&&s.data()?.status==='pending')return;await setDoc(ref,{communityId:cid,userId:user.uid,username:user.username,status:'pending',createdAt:serverTimestamp(),updatedAt:serverTimestamp()});if(community.ownerId&&community.ownerId!==user.uid)void notify(community.ownerId,{type:'community_join_request',actorId:user.uid,actorUsername:user.username||'',actorName:user.name||user.username||'User',message:`requested to join ${community.name}`,targetType:'profile',targetId:cid}).catch(()=>{});}
export async function getCommunityJoinRequest(cid:string,uid:string){const s=await getDoc(doc(db,'communities',cid,'joinRequests',uid));return s.exists()?({id:s.id,...s.data()} as CommunityJoinRequest):null;}
export async function getCommunityJoinRequests(cid:string){const s=await getDocs(query(collection(db,'communities',cid,'joinRequests'),where('status','==','pending'),limit(200)));return s.docs.map(d=>({id:d.id,...d.data()} as CommunityJoinRequest));}
export async function reviewCommunityJoinRequest(cid:string,uid:string,approve:boolean,actorId:string){const c=await getCommunity(cid);if(!c)throw new Error('Community not found.');const actorRole=await getCommunityRole(cid,actorId);if(c.ownerId!==actorId&&!['owner','moderator'].includes(String(actorRole||'')))throw new Error('You cannot review join requests.');const req=await getCommunityJoinRequest(cid,uid);if(!req||req.status!=='pending')throw new Error('Join request is no longer pending.');const userSnap=await getDoc(doc(db,'users',uid));if(userSnap.data()?.isBlocked===true)throw new Error('This user cannot join the community.');if(approve){await runTransaction(db,async tx=>{const memberRef=doc(db,'communities',cid,'members',uid);const requestRef=doc(db,'communities',cid,'joinRequests',uid);const communityRef=doc(db,'communities',cid);const member=await tx.get(memberRef);const requestSnap=await tx.get(requestRef);const communitySnap=await tx.get(communityRef);if(!communitySnap.exists()||!requestSnap.exists()||requestSnap.data()?.status!=='pending')throw new Error('Join request is no longer pending.');if(member.exists()){tx.update(requestRef,{status:'approved',updatedAt:serverTimestamp()});return;}tx.set(memberRef,{uid,username:userSnap.data()?.username||req.username||'',role:'member',createdAt:serverTimestamp(),updatedAt:serverTimestamp()});tx.update(requestRef,{status:'approved',updatedAt:serverTimestamp()});tx.update(communityRef,{membersCount:increment(1),updatedAt:serverTimestamp()});});}else await updateDoc(doc(db,'communities',cid,'joinRequests',uid),{status:'rejected',updatedAt:serverTimestamp()});void notify(uid,{type:approve?'community_join_approved':'community_join_rejected',actorId,actorUsername:'',actorName:'Community moderation',message:approve?`Your request to join ${c.name} was approved.`:`Your request to join ${c.name} was rejected.`,targetType:'profile',targetId:cid}).catch(()=>{});}
export async function inviteCommunityMember(cid:string,handleInput:string,actorId:string){
  const c=await getCommunity(cid);
  if(!c)throw new Error('Community not found.');
  const role=await getCommunityRole(cid,actorId);
  if(!role||!['owner','moderator'].includes(role))throw new Error('You cannot invite members.');
  const clean=String(handleInput||'').trim().replace(/^@/,'').toLowerCase().replace(/[^a-z0-9_]/g,'');
  if(clean.length<3||clean.length>30)throw new Error('Enter a valid @handle.');
  const handleSnap=await getDoc(doc(db,'usernames',clean));
  if(!handleSnap.exists())throw new Error(`User @${clean} not found.`);
  const inviteeId=String(handleSnap.data()?.uid||'');
  if(!inviteeId)throw new Error(`User @${clean} not found.`);
  if(inviteeId===actorId)throw new Error('You cannot invite yourself.');
  if((await getDoc(doc(db,'communities',cid,'members',inviteeId))).exists())throw new Error('This user is already a member.');
  const existing=await getDocs(query(collection(db,'communities',cid,'invitations'),where('inviteeId','==',inviteeId),where('status','==','pending'),limit(1)));
  if(!existing.empty)throw new Error('A pending invitation already exists.');
  const inviterProfile=await profile(actorId);
  const ref=doc(db,'communities',cid,'invitations',inviteeId);
  await setDoc(ref,{communityId:cid,inviteeId,inviteeUsername:clean,inviterId:actorId,inviterUsername:String(inviterProfile?.username||''),status:'pending',createdAt:serverTimestamp(),updatedAt:serverTimestamp()});
  void notify(inviteeId,{type:'community_invitation',actorId,actorUsername:String(inviterProfile?.username||''),actorName:String(inviterProfile?.displayName||'Community invitation'),message:`You were invited to join ${c.name}.`,targetType:'profile',targetId:cid}).catch(()=>{});
  return ref.id;
}
export async function getCommunityInvitations(cid:string,inviteeId?:string){const base=collection(db,'communities',cid,'invitations');const s=await getDocs(inviteeId?query(base,where('inviteeId','==',inviteeId),limit(100)):query(base,orderBy('createdAt','desc'),limit(100)));return s.docs.map(d=>({id:d.id,...d.data(),createdAt:date(d.data()?.createdAt)} as CommunityInvitation));}
export async function respondToCommunityInvitation(cid:string,id:string,accept:boolean,user:CommunityUser){if(!user?.uid)throw new Error('Sign in required.');const ref=doc(db,'communities',cid,'invitations',id);const communityRef=doc(db,'communities',cid);const memberRef=doc(db,'communities',cid,'members',user.uid);let inviterId='';await runTransaction(db,async tx=>{const inv=await tx.get(ref);const community=await tx.get(communityRef);const member=await tx.get(memberRef);if(!inv.exists()||inv.data()?.inviteeId!==user.uid||inv.data()?.status!=='pending')throw new Error('Invitation is no longer valid.');if(!community.exists())throw new Error('Community not found.');inviterId=String(inv.data()?.inviterId||'');if(!accept){tx.update(ref,{status:'declined',updatedAt:serverTimestamp()});return;}if(!member.exists()){tx.set(memberRef,{uid:user.uid,username:user.username,role:'member',invitationId:id,createdAt:serverTimestamp(),updatedAt:serverTimestamp()});tx.update(communityRef,{membersCount:increment(1),updatedAt:serverTimestamp()});}tx.update(ref,{status:'accepted',updatedAt:serverTimestamp()});});if(inviterId)void notify(inviterId,{type:accept?'community_invitation_accepted':'community_invitation_declined',actorId:user.uid,actorUsername:user.username||'',actorName:user.name||user.username||'User',message:accept?'accepted your community invitation.':'declined your community invitation.',targetType:'profile',targetId:cid}).catch(()=>{});return accept;}
export async function revokeCommunityInvitation(cid:string,id:string,actorId:string){const role=await getCommunityRole(cid,actorId);if(!role||!['owner','moderator'].includes(role))throw new Error('You cannot revoke invitations.');const ref=doc(db,'communities',cid,'invitations',id);const s=await getDoc(ref);if(!s.exists()||s.data()?.status!=='pending')throw new Error('Invitation is no longer pending.');await updateDoc(ref,{status:'revoked',updatedAt:serverTimestamp()});void notify(String(s.data()?.inviteeId||''),{type:'community_invitation_revoked',actorId,actorUsername:'',actorName:'Community moderation',message:'A community invitation was revoked.',targetType:'profile',targetId:cid}).catch(()=>{});}
export async function getCommunityWiki(cid:string){const s=await getDocs(query(collection(db,'communities',cid,'wiki'),orderBy('updatedAt','desc'),limit(100)));return s.docs.map(d=>({id:d.id,...d.data()} as CommunityWikiPage));}
export async function saveCommunityWikiPage(cid:string,input:{id?:string;title:string;slug:string;content:string;authorId:string}){const role=await getCommunityRole(cid,input.authorId);if(!role||!['owner','moderator'].includes(role))throw new Error('You cannot edit the community wiki.');const ref=input.id?doc(db,'communities',cid,'wiki',input.id):doc(collection(db,'communities',cid,'wiki'));await setDoc(ref,{title:input.title.slice(0,120),slug:input.slug.slice(0,120),content:input.content.slice(0,20000),authorId:input.authorId,createdAt:serverTimestamp(),updatedAt:serverTimestamp(),archived:false},{merge:true});return ref.id;}
export async function deleteCommunityWikiPage(cid:string,id:string,actorId:string){const role=await getCommunityRole(cid,actorId);if(!role||!['owner','moderator'].includes(role))throw new Error('You cannot delete wiki pages.');await updateDoc(doc(db,'communities',cid,'wiki',id),{archived:true,updatedAt:serverTimestamp()});}
export async function getCommunityFaq(cid:string){const s=await getDocs(query(collection(db,'communities',cid,'faq'),orderBy('order','asc'),limit(100)));return s.docs.map(d=>({id:d.id,...d.data()} as CommunityFaq));}
export async function saveCommunityFaq(cid:string,input:{id?:string;question:string;answer:string;order:number;actorId:string}){const role=await getCommunityRole(cid,input.actorId);if(!role||!['owner','moderator'].includes(role))throw new Error('You cannot edit the FAQ.');const ref=input.id?doc(db,'communities',cid,'faq',input.id):doc(collection(db,'communities',cid,'faq'));await setDoc(ref,{question:input.question.slice(0,300),answer:input.answer.slice(0,5000),order:Math.max(0,Math.floor(input.order)),updatedBy:input.actorId,createdAt:serverTimestamp(),updatedAt:serverTimestamp()},{merge:true});return ref.id;}
export async function getCommunityResources(cid:string){const s=await getDocs(query(collection(db,'communities',cid,'resources'),orderBy('createdAt','desc'),limit(100)));return s.docs.map(d=>({id:d.id,...d.data()} as CommunityResource));}
export async function saveCommunityResource(cid:string,input:{id?:string;title:string;description?:string;url:string;category?:string;creatorId:string}){const role=await getCommunityRole(cid,input.creatorId);if(!role||!['owner','moderator'].includes(role))throw new Error('You cannot edit resources.');if(!/^https?:\/\//i.test(input.url))throw new Error('Resource URL must use http or https.');const ref=input.id?doc(db,'communities',cid,'resources',input.id):doc(collection(db,'communities',cid,'resources'));await setDoc(ref,{title:input.title.slice(0,120),description:(input.description||'').slice(0,500),url:input.url.slice(0,2000),category:(input.category||'').slice(0,80),creatorId:input.creatorId,createdAt:serverTimestamp()},{merge:true});return ref.id;}
export async function getCommunityAnalytics(cid:string,days=7){const windowDays=[7,30,90].includes(days)?days:7;const cutoff=Date.now()-windowDays*86400000;const [members,posts,questionsSnap]=await Promise.all([getDocs(query(collection(db,'communities',cid,'members'),limit(1000))),getDocs(query(collection(db,'communities',cid,'posts'),limit(1000))),getDocs(query(collection(db,'questions'),where('relatedCommunityId','==',cid),limit(1000)))]);const memberRows=members.docs.map(d=>d.data());const postRows=posts.docs.map(d=>d.data());const activeMembers=memberRows.filter(d=>d?.banned!==true).length;const newMembers=memberRows.filter(d=>new Date(date(d?.createdAt)).getTime()>=cutoff).length;const activeMemberIds=new Set(memberRows.filter(d=>d?.banned!==true&&new Date(date(d?.updatedAt||d?.createdAt)).getTime()>=cutoff).map(d=>String(d?.uid||'')));const visiblePosts=postRows.filter(d=>!d?.isArchived);const discussions=visiblePosts.filter(d=>String(d?.postType||'discussion')!=='blog').length;const questions=questionsSnap.docs.filter(d=>d.data()?.moderationState!=='REMOVED');const questionCount=questions.length;const answerCount=questions.reduce((sum,d)=>sum+Math.max(0,Number(d.data()?.answersCount||0)),0);const comments=visiblePosts.reduce((sum,d)=>sum+Math.max(0,Number(d?.commentsCount||0)),0);const engagement=visiblePosts.reduce((sum,d)=>sum+Math.max(0,Number(d?.score||0))+Math.max(0,Number(d?.commentsCount||0)),0);return {members:activeMembers,newMembers7:newMembers,membersGrowth:newMembers,activeMembers:activeMemberIds.size,posts:visiblePosts.length,discussions,questions:questionCount,answers:answerCount,comments,engagement,windowDays,generatedAt:new Date().toISOString()};}
...
const questionPublic=(data:any,idv:string):SocialQuestion=>{ const attachments=Array.isArray(data.attachments)?data.attachments:[]; const mediaUrls=Array.isArray(data.mediaUrls)?data.mediaUrls:attachments; return {id:idv,title:String(data.title||''),details:String(data.details||''),authorId:data.isAnonymous?undefined:data.authorId,authorUsername:data.isAnonymous?undefined:data.authorUsername,authorName:data.isAnonymous?'Anonymous':(data.authorName||'Anonymous'),authorAvatar:data.isAnonymous?'':(data.authorAvatar||''),isAnonymous:!!data.isAnonymous,topics:Array.isArray(data.topics)?data.topics:[],attachments,mediaUrls,coverImage:data.coverImage||mediaUrls[0]||undefined,coverImageAlt:data.coverImageAlt||undefined,relatedArticleSlug:data.relatedArticleSlug||undefined,relatedCommunityId:data.relatedCommunityId||undefined,followersCount:Number(data.followersCount||0),answersCount:Number(data.answersCount||0),upvotesCount:Number(data.upvotesCount||0),bestAnswerId:data.bestAnswerId||undefined,moderationState:(data.moderationState||'VISIBLE') as QuestionModerationState,duplicateOf:data.duplicateOf||undefined,createdAt:date(data.createdAt),updatedAt:date(data.updatedAt)}; };
...
export async function createQuestion(user:CommunityUser,title:string,details:string,topics:string[],options:{attachments?:string[];mediaUrls?:string[];coverImage?:string;coverImageAlt?:string;relatedArticleSlug?:string;relatedCommunityId?:string;isAnonymous?:boolean}={}){ const qid=id(); const anonymous=!!options.isAnonymous; const data:any={title:title.trim().slice(0,256),details:details.trim().slice(0,100000),authorId:user.uid,authorUsername:user.username,authorName:user.displayName,authorAvatar:user.photoURL||'',isAnonymous:anonymous,topics:cleanTopics(topics),attachments:(options.attachments||options.mediaUrls||[]).slice(0,8),mediaUrls:(options.mediaUrls||options.attachments||[]).slice(0,8),coverImage:options.coverImage||((options.mediaUrls||options.attachments||[])[0]||undefined),coverImageAlt:options.coverImageAlt||'',relatedArticleSlug:options.relatedArticleSlug?.trim().slice(0,180)||'',relatedCommunityId:options.relatedCommunityId?.trim().slice(0,120)||'',followersCount:0,answersCount:0,upvotesCount:0,moderationState:'VISIBLE',createdAt:serverTimestamp(),updatedAt:serverTimestamp()}; await setDoc(doc(db,'questions',qid),data); void emitActivityEvent({type:'question_create',targetId:qid,targetType:'question',source:'question'}).catch(()=>{}); return questionPublic({...data,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()},qid); }
export async function updateQuestion(qid:string,uid:string,title:string,details:string,options:{topics?:string[];attachments?:string[];mediaUrls?:string[];coverImage?:string;coverImageAlt?:string;relatedArticleSlug?:string;relatedCommunityId?:string}={}){const q=await getDoc(doc(db,'questions',qid));if(!q.exists())throw new Error('Question not found.');const admin=await resolveMasterAccess();if(q.data().authorId!==uid&&!admin)throw new Error('You cannot edit this question.');const patch:any={title:title.trim().slice(0,256),details:details.trim().slice(0,100000),updatedAt:serverTimestamp()};if(options.topics)patch.topics=cleanTopics(options.topics);if(options.attachments){patch.attachments=options.attachments.slice(0,8);patch.mediaUrls=options.attachments.slice(0,8);if(patch.mediaUrls[0]&&!options.coverImage)patch.coverImage=patch.mediaUrls[0];}if(options.mediaUrls){patch.mediaUrls=options.mediaUrls.slice(0,8);if(!options.attachments)patch.attachments=options.mediaUrls.slice(0,8);if(patch.mediaUrls[0]&&!options.coverImage)patch.coverImage=patch.mediaUrls[0];}if(options.coverImage!==undefined)patch.coverImage=options.coverImage||'';if(options.coverImageAlt!==undefined)patch.coverImageAlt=options.coverImageAlt||'';if(options.relatedArticleSlug!==undefined)patch.relatedArticleSlug=options.relatedArticleSlug.trim().slice(0,180);if(options.relatedCommunityId!==undefined)patch.relatedCommunityId=options.relatedCommunityId.trim().slice(0,120);await updateDoc(q.ref,patch);}
...
export async function voteQuestion(qid:string,uid:string){const ref=doc(db,'questions',qid,'questionVotes',uid);const qRef=doc(db,'questions',qid);const existing=await getDoc(ref);const b=writeBatch(db);if(existing.exists()){b.delete(ref);b.update(qRef,{upvotesCount:increment(-1)});await b.commit();void emitActivityEvent({type:'unlike',targetId:qid,targetType:'question',source:'question',metadata:{action:'unvote'}}).catch(()=>{});return false;}b.set(ref,{uid,createdAt:serverTimestamp()});b.update(qRef,{upvotesCount:increment(1),updatedAt:serverTimestamp()});await b.commit();void emitActivityEvent({type:'like',targetId:qid,targetType:'question',source:'question',metadata:{action:'upvote'}}).catch(()=>{});return true;}
