...
export interface CommunityInvitation { id:string; communityId:string; inviteeId:string; inviterId:string; status:'pending'|'accepted'|'declined'|'revoked'; createdAt:string; }
...
export async function inviteCommunityMember(cid:string,inviteeId:string,actorId:string){const c=await getCommunity(cid);if(!c)throw new Error('Community not found.');const role=await getCommunityRole(cid,actorId);if(!role||!['owner','moderator'].includes(role))throw new Error('You cannot invite members.');if(!inviteeId||inviteeId===actorId)throw new Error('Choose another user.');const target=await getDoc(doc(db,'users',inviteeId));if(!target.exists())throw new Error('Invited user not found.');if(target.data()?.isBlocked===true)throw new Error('This user cannot be invited.');if((await getDoc(doc(db,'communities',cid,'members',inviteeId))).exists())throw new Error('This user is already a member.');const existing=await getDocs(query(collection(db,'communities',cid,'invitations'),where('inviteeId','==',inviteeId),where('status','==','pending'),limit(1)));if(!existing.empty)throw new Error('A pending invitation already exists.');const ref=doc(db,'communities',cid,'invitations',inviteeId);await setDoc(ref,{communityId:cid,inviteeId,inviterId:actorId,status:'pending',createdAt:serverTimestamp(),updatedAt:serverTimestamp()});void notify(inviteeId,{type:'community_invitation',actorId,actorUsername:'',actorName:'Community invitation',message:`You were invited to join ${c.name}.`,targetType:'profile',targetId:cid}).catch(()=>{});return ref.id;}
