...
export const seriesToContent = (s: Series): ContentEntity => normalizeContent({ id: s.id, type: 'series', authorId: s.ownerId, title: s.title, content: s.description, excerpt: s.description, tags: s.tags, status: s.status === 'archived' ? 'archived' : (s.status || 'published'), visibility: s.visibility || 'public', createdAt: s.createdAt, updatedAt: s.updatedAt, stats: { views: s.viewsCount }, sourceCollection: 'series', sourcePath: `series/${s.id}` });
export const userToContent = (u: CommunityUser): ContentEntity => normalizeContent({ id: u.uid, type: 'user', authorId: u.uid, title: u.displayName || u.username, content: u.bio, tags: [], topics: [], visibility: 'public', status: u.isBlocked || u.isSuspended ? 'archived' : 'published', createdAt: u.createdAt, updatedAt: u.updatedAt, sourceCollection: 'users', sourcePath: `users/${u.uid}` });
