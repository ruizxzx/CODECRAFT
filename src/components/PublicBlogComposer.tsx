  ...
  const publish = async () => {
    ...
    try {
      ...
      if (initialPost) {
        ...
        setStatus('SCRPT changes saved to Firebase and confirmed.');
      } else if (community) {
        post = await createCommunityPost(community.id, userProfile, cleanTitle, body, {
          postType: 'blog',
          ...metadata,
        });
        setStatus(post.publishStatus === 'existing' ? 'Existing cloud copy restored.' : 'SCRPT published and confirmed in Firebase.');
      } else {
        ...
        setStatus((post as any)._publishStatus === 'existing' ? 'Existing cloud copy restored.' : 'SCRPT published and confirmed in Firebase.');
      }
      ...
      <div className="max-w-5xl mx-auto bg-white border-4 border-black neo-shadow-lg">
        <div className="bg-[var(--color-primary)] border-b-4 border-black p-4 flex justify-between items-center">
          <div>
            ...
            <h2 className="font-display font-black text-2xl uppercase">{initialPost ? 'EDIT SCRPT' : 'WRITE A SCRPT'}{community ? ' · COMMUNITY' : ''}</h2>
          </div>
        ...
        <div className="p-5 space-y-5">
          <div className="grid lg:grid-cols-2 gap-3">
            <input value={title} onChange={(e) => setTitle(e.target.value.slice(0, 256))} placeholder="SCRPT title" className="border-2 border-black p-3 font-display font-bold text-xl" />
          ...
          {preview && <div className="border-4 border-black p-5 space-y-4 bg-white">
            ...
            <h1 className="font-display font-black text-3xl uppercase">{title || 'UNTITLED SCRPT'}</h1>
          ...
          <div className="flex flex-wrap justify-between items-center gap-2 border-t-2 border-black pt-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button onClick={() => void publish()} disabled={publishing} className="border-2 border-black bg-[var(--color-primary)] px-5 py-2 font-mono text-xs font-black disabled:opacity-50">{publishing ? (initialPost ? 'SAVING…' : 'PUBLISHING…') : (initialPost ? 'SAVE CHANGES' : 'PUBLISH SCRPT')}</button>
            </div>
          </div>
        </div>
      </div>
