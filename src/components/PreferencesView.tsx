  ...
  return <section className="max-w-3xl mx-auto px-4 sm:px-6 py-10 sm:py-16">
    ...
    <div className="mb-6 border-4 border-black bg-white p-4 sm:p-5 neo-shadow">
      <div className="flex items-center justify-between gap-4">
        ...
        <button onClick={toggleTheme} disabled={themeSaving} className={`w-16 h-9 border-2 border-black p-1 ${theme === 'dark' ? 'bg-black' : 'bg-neutral-200'}`} aria-label="Toggle dark mode"><span className={`block w-6 h-6 bg-[var(--color-primary)] border-2 border-black transition-transform ${theme === 'dark' ? 'translate-x-7' : 'translate-x-0'}`}/></button>
      </div>
