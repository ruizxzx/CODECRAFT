    ...
    <div className="w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="bg-white border-4 border-black neo-shadow-lg overflow-hidden mb-8">
        ...
        <div className="px-6 sm:px-10 pb-8 relative">
          ...
          {isEditing ? <form onSubmit={handleSaveProfile} className="space-y-4 max-w-xl"><div className="space-y-1"><label className="font-mono text-[10px] font-bold uppercase">Unique @Handle</label><input value={handleInput} onChange={e => setHandleInput(e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, "").slice(0, 30))} maxLength={30} placeholder="your_handle (optional)" className="w-full px-3 py-2 border-2 border-black font-mono text-sm" /><p className="font-mono text-[9px] text-neutral-500">Leave blank to stay unclaimed. Handles are global and unique.</p></div><div className="space-y-1"><label className="font-mono text-[10px] font-bold uppercase">Display Name</label><input value={displayNameInput} onChange={e => setDisplayNameInput(e.target.value)} maxLength={64} className="w-full px-3 py-2 border-2 border-black font-display font-bold text-sm" /></div><div className="space-y-1"><label className="font-mono text-[10px] font-bold uppercase">Profile Picture URL</label><div className="flex flex-col sm:flex-row gap-2"><input type="url" value={photoUrlInput} onChange={e => setPhotoUrlInput(e.target.value)} placeholder="https://example.com/your-profile-picture.jpg" className="flex-1 px-3 py-2 border-2 border-black font-mono text-xs" /><MediaUploadButton folder="profile" accept="image/jpeg,image/png,image/webp,image/gif,image/avif" label="UPLOAD AVATAR" compact cropAspect="1:1" cropShape="circle" outputWidth={800} outputHeight={800} onUploaded={(url) => setPhotoUrlInput(url)} /></div><p className="font-mono text-[9px] text-neutral-500 uppercase">Upload directly to OFFSCRPT media storage or paste an existing public image URL.</p></div><div className="space-y-1"><label className="font-mono text-[10px] font-bold uppercase">Profile Cover Image URL</label><div className="flex flex-col sm:flex-row gap-2"><input type="url" value={coverUrlInput} onChange={e => setCoverUrlInput(e.target.value)} placeholder="https://example.com/cover.jpg" className="flex-1 px-3 py-2 border-2 border-black font-mono text-xs" /><MediaUploadButton folder="profile" accept="image/jpeg,image/png,image/webp,image/gif,image/avif" label="UPLOAD COVER" compact cropAspect="2:1" cropShape="rect" outputWidth={1600} outputHeight={800} onUploaded={(url) => setCoverUrlInput(url)} /></div><p className="font-mono text-[9px] text-neutral-500 uppercase">Optional public image URL or upload a cover directly to OFFSCRPT media storage.</p></div><div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
