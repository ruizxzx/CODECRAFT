...
export async function uploadMedia(file: File, folder: 'profile' | 'articles' | 'posts' | 'videos' | 'attachments' | 'answers' | 'carousel' | 'users' | 'site', onProgress?: (progress: number) => void, options?: { targetUid?: string }): Promise<MediaUploadResult> {
