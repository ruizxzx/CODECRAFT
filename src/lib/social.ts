...
export async function reviewCommunityJoinRequest(cid:string,uid:string,approve:boolean,actorId:string){const c=await getCommunity(cid);if(!c||c.ownerId!==actorId&&!(await getCommunityRole(cid,actorId)?.then(r=>r==='moderator'||r==='owner')))throw new Error('You cannot review join requests.');const req=await getCommunityJoinRequest(cid,uid);if(!req)throw new Error('Join request not found.');if(approve){const userSnap=await getDoc(doc(db,'users',uid));const pdata:any=userSnap.exists()?userSnap.data():{};const b=writeBatch(db);b.set(doc(db,'communities',cid,'members',uid),{uid,username:pdata.username||req.username||'',role:'member',createdAt:serverTimestamp(),updatedAt:serverTimestamp()});b.update(doc(db,'communities',cid,'joinRequests',uid),{status:'approved',updatedAt:serverTimestamp()});b.update(doc(db,'communities',cid),{membersCount:increment(1),updatedAt:serverTimestamp()});await b.commit();}else await updateDoc(doc(db,'communities',cid,'joinRequests',uid),{status:'rejected',updatedAt:serverTimestamp()});}
export async function inviteCommunityMember(cid:string,inviteeId:string,actorId:string){const role=await getCommunityRole(cid,actorId);if(!role||!['owner','moderator'].includes(role))throw new Error('You cannot invite members.');const ref=doc(collection(db,'communities',cid,'invitations'));await setDoc(ref,{communityId:cid,inviteeId,inviterId:actorId,status:'pending',createdAt:serverTimestamp()});}
export async function getCommunityInvitations(cid:string,inviteeId?:string){const base=collection(db,'communities',cid,'invitations');const s=await getDocs(inviteeId?query(base,where('inviteeId','==',inviteeId),limit(100)):query(base,orderBy('createdAt','desc'),limit(100)));return s.docs.map(d=>({id:d.id,...d.data()} as CommunityInvitation));}
export async function respondToCommunityInvitation(cid:string,id:string,accept:boolean,user:CommunityUser){const ref=doc(db,'communities',cid,'invitations',id);const s=await getDoc(ref);if(!s.exists()||s.data()?.inviteeId!==user.uid||s.data()?.status!=='pending')throw new Error('Invitation is no longer valid.');if(accept){await joinCommunity(cid,user);await updateDoc(ref,{status:'accepted'});}else await updateDoc(ref,{status:'declined'});}
export async function revokeCommunityInvitation(cid:string,id:string,actorId:string){const role=await getCommunityRole(cid,actorId);if(!role||!['owner','moderator'].includes(role))throw new Error('You cannot revoke invitations.');await updateDoc(doc(db,'communities',cid,'invitations',id),{status:'revoked'});}
...
export async function getCommunityAnalytics(cid:string){const [members,posts]=await Promise.all([getDocs(query(collection(db,'communities',cid,'members'),limit(500))),getDocs(query(collection(db,'communities',cid,'posts'),limit(500)))]);const activeMembers=members.docs.filter(d=>d.data()?.banned!==true).length;const now=Date.now();const newMembers7=members.docs.filter(d=>now-new Date(date(d.data()?.createdAt)).getTime()<=7*86400000).length;return {members:activeMembers,newMembers7,posts:posts.docs.filter(d=>!d.data()?.isArchived).length,generatedAt:new Date().toISOString()};}
