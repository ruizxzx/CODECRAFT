...
if(!profile.includes('updateCommunityProfile(profile.uid')) failures.push('public profile editor save missing');
if(!community.includes('adminChangeUserHandle')) failures.push('admin handle change operation missing');
if(!profile.includes("onNavigate('community_profile', p.username)")) failures.push('public profile handle redirect missing');
if(!community.includes('handleAliases')) failures.push('legacy handle alias routing missing');
