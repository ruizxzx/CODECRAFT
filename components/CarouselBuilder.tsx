...
export const CarouselBuilder: React.FC<Props> = ({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative aspect-[21/9] border-4 border-black overflow-hidden bg-black cursor-crosshair"
        onClick={(event) => {
        ...
        {mode === 'image' && imageUrl && <img src={imageUrl} alt="Carousel preview" className="absolute inset-0 w-full h-full object-cover" style={{ objectPosition: `${positionX}% ${positionY}%`, transform: `scale(${zoom / 100})` }} />}
        {mode === 'image' && !imageUrl && <div className="absolute inset-0 grid place-items-center text-white font-mono text-xs">ADD AN IMAGE URL</div>}
