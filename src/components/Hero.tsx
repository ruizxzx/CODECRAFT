...
export const Hero: React.FC<HeroProps> = ({ onNavigate, postsCount, siteConfig }) => {
  ...
  useEffect(() => { let active = true; optimizedGetCount('hero:users', () => getCountFromServer(collection(db, 'publicProfiles')), 600_000).then(count => { if (active) setReaderCount(count); }).catch((error) => console.warn('Hero reader count unavailable:', error)); return () => { active = false; }; }, []);
